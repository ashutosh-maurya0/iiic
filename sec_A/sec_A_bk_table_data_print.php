...
<body>
    ...
    <form method="post" enctype="multipart/form-data">
      ...
      <div id="reviewModal" class="modal">
        <div class="modal-content">
            ...
            <button onclick="printReview()">Print Review</button>
        </div>
      </div>
      <table id="mytab1">
        <tr>
    ...
    <script>
      ...
      function reviewData() {
        // Customize this object to include all relevant form fields
        var formData = {
         'CIN': document.getElementById('cin').value,
         'name': document.getElementById('name').value,
         'incorp_date' : document.getElementById('incorp_date').value,
          // Add other fields as needed
        };
        var reviewContent = '<ul>';
          for (var key in formData) {
            if (formData.hasOwnProperty(key)) {
              reviewContent += '<li><strong>' + key + ':</strong> ' + formData[key] + '</li>';
            }
          }
          reviewContent += '</ul>';
...
for (var i = 0; i < tableData.length; i++) {
  ...
  printWindow.document.write('</tr>');
    ...
    <div class="mb-3">
        <button type="button" class="btn btn-primary" onclick="reviewData()">Review Data</button>
        <button type="submit" class="btn btn-success" onclick="return confirmSubmission()">Final Submission</button>
    </div>
